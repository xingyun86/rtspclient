	...
	MyRTPSession()
{
    TunnellingSock = 0;
#ifdef _MSC_VER
	InitializeCriticalSectionAndSpinCount(&SocketMutex, 0);
#else
    ...
    *size = 0;
	...
	do {
		...
		if (!GotoFirstSourceWithData()) {
			EndDataAccess();
#ifdef _MSC_VER
			std::this_thread::sleep_for(std::chrono::microseconds(USLEEP_UNIT));
#else
			usleep(USLEEP_UNIT);
#endif
			UsleepTimes--;
...
#ifdef _MSC_VER
    if(TRUE != TryEnterCriticalSection(&SocketMutex)) {
#else
